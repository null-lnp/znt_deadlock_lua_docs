...
**Scourge** also uses a confirmed two-stage flow. After selection, it holds the `alt_cast` modifier for each command while Scourge remains selected, allowing the game to apply the friendly-target effect to the local player without depending on a one-frame modifier tap. **Infuser** is a no-target activation gated by a configurable local-health percentage. **Fleetfoot** is a no-target mobility activation that defaults to Combo-key use. All three resolve their current inventory slot and validate readiness from `active_item()` rather than assuming a purchase key.
