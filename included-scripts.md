...
The cursor status distinguishes **HOOK NOT READY**, **HOOK REQUESTED**, and **HOOKING**. While loaded, the sample records every copied `entity_created` event before applying its Bebop-only logic gate. Each diagnostic includes the handle, entity index, designer name (or `<unavailable>`), schema class (or `<unavailable>`), `hero_match`, and combo state. This keeps callback delivery visible even when local-hero resolution fails. Hook tracking remains Bebop-only, and matching `entity_deleted` diagnostics remain limited to entities captured during the short Hook-launch trace so unrelated lifecycle activity cannot change Hook state.
