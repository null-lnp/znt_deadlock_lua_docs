...
Activator is an item-rule host with one compact **Activator** page. Echo Shard, Slowing Hex, Scourge, Infuser, and Fleetfoot render as real-icon sections inside that page instead of additional sidebar tabs. It owns one configurable combo key. Each item independently chooses **Always** or **Combo key** and can require an enemy inside its own configurable range, so no rule imports another script's shared mode.
