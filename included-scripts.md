...
**Scourge** also uses a confirmed two-stage flow. After selection, it emits one `alt_cast` press edge and preserves the modifier only on later selected commands, allowing the game to apply the friendly-target effect to the local player. **Infuser** is a no-target activation gated by a configurable local-health percentage. **Fleetfoot** is a no-target mobility activation that defaults to Combo-key use. All three resolve their current inventory slot and validate readiness from `active_item()` rather than assuming a purchase key.
