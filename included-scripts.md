...
**Auto cast Ability 2** is independent of the combo key. When enabled, it uses the live learned/readiness state and casts as soon as Victor's health falls strictly below the configured percentage; a pending request waits for cooldown confirmation before another cast is allowed.
