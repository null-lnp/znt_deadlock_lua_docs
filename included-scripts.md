...
Use the detached **Lua Console** while reproducing an issue. A detection entry without a later `parry:` entry means the input request was intentionally gated rather than lost. The SDK resolves remote hold-melee abilities from owned entity-list entries when the player component omits them, so an active light state can trigger before the later swing sound. Heavy attacks must face the local player even at point blank. Light attacks require the local player to be inside the attacker's forward sector and no farther than 200 world units. The sample uses an effective half-angle of at least 45°, while honoring wider live `MeleeHalfAngle` values. `Ability.Melee.Impact.Player` is intentionally not used for detection because it arrives after the hit.
