...
The full ability snapshot is resolved from either the player's component vector or an owned hold-melee entity in the replicated entity list. When the melee-ability cooldown-start timestamp advances before owner-only detailed melee fields replicate, the SDK emits a minimal `"cooldown"` snapshot with that timestamp in `attack_started_at`. The SDK latches the observed value change for the current command window, so interpolation cannot discard a newly received cast merely because its remote simulation timestamp already looks old locally. A stale timestamp seen for the first time is not treated as a new attack. Practice actors that omit their client melee entity use the authoritative cooldown timestamp from the local listen server. Recognized modifiers remain the conservative fallback.
